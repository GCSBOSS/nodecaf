...
module.exports = class Nodecaf {
    ...
    setup(...objectOrPath){
        ...
        this.log = new Logger(this, this.conf.log);

        if(this.conf.delay)
            this.log.warn('`conf.delay` is deprecated. This configuration will be dropped on `v0.14.0`.');

        if(this.conf.port)
            this.log.warn('`conf.port` is deprecated. This configuration will be dropped on `v0.14.0`. Use `opts.http/´ instead.');

        if(this.conf.log.only)
            this.log?.warn('Setting `conf.log.only` is deprecated. This configuration will be dropped on `v0.14.0`. Use external filtering instead.');

        if(this.conf.log.except)
            this.log?.warn('Setting `conf.log.except` is deprecated. This configuration will be dropped on `v0.14.0`. Use external filtering instead.');
    }
