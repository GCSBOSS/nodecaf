const { layerConf } = require('./conf');
const { startServer } = require('./http');
const API = require('./api');
const { Readable, Writable } = require('stream');
const { getDataTypeFromContentType, getContentTypeFromData: getContentTypeFromDataType, parseBuffer, bytesToString, getDataFromBytesAndTypes } = require('./types');
const { Logger } = require('./logger');
const nativeModule = require('./native');

function findPkgInfo(){
    try{
        return require(module.parent.path + '/../package.json');
    }
    catch{
        /* istanbul ignore next */
        return { name: 'Untitled', version: '0.0.0' };
    }
}

function retryShortly(fn){
    return new Promise(done => setTimeout(() => fn().then(done), 1000));
}

function validateOpts(opts){
    if(typeof opts != 'object')
        throw new TypeError('Options argument must be an object');

    this._websocket = opts.websocket;
    this._routes = opts.routes;
    this._startup = opts.startup;
    this._shutdown = opts.shutdown;
    this._serverBuilder = opts.server;
    this._http = opts.http;

    const { name, version } = findPkgInfo();
    this._name = opts.name ?? name;
    this._version = opts.version ?? version;

    if(opts.routes && !Array.isArray(this._routes))
        throw new TypeError('Routes must be an array');

    if(opts.startup && typeof this._startup != 'function')
        throw new TypeError('Startup handler must be a function');

    if(opts.shutdown && typeof this._shutdown != 'function')
        throw new TypeError('Shutdown handler must be a function');

    if(opts.server && typeof this._serverBuilder != 'function')
        throw new TypeError('Server builder must be a function');

    if(opts.http && typeof this._http != 'number')
        throw new TypeError('HTTP port number must be of type \'number\'');
}

module.exports = class Nodecaf {

    /** @type {import('./native').NativeServerHandles} */
    #_serverHandles;

    constructor(opts = {}){
        validateOpts.call(this, opts);

        this._autoParseBody = opts.autoParseBody ?? false;
        this._reqBodyTimeout = opts.reqBodyTimeout ?? 3000;

        this.call = (fn, ...args) => fn.call(this, { ...this.global, ...this }, ...args);

        this.conf = {};
        this.state = 'standby';

        this.setup(opts.conf);

        this._api = new API(this, opts.routes ?? []);
    }

    setup(...objectOrPath){
        this.conf = layerConf(this.conf, ...objectOrPath);
        this.conf.log = this.conf.log ?? {};
        
        this.log = new Logger({
            level: this.conf.log.level,
            disabled: this.conf.log === false,
            appName: this._name
        });
    }

    async start(){

        if(this.state in { running: 1, starting: 1 })
            return this.state;
        if(this.state == 'stopping')
            return await retryShortly(() => this.start());

        this.state = 'starting';

        this.global = {};

        if(this._startup)
            this.log.debug({ type: 'app' }, 'Starting up %s...', this._name);

        // Handle exceptions in user code to maintain proper app state
        try{
            await this._startup?.(this);
        }
        catch(err){
            this.state = 'stuck';
            await this.stop().catch(Function.prototype);
            throw err;
        }

        if(this._http){
            this.#_serverHandles = await nativeModule.createServer(this, this._http, this._websocket);
            this.log.info({ type: 'app' }, '%s has started', this._name);
        }

        return this.state = 'running';
    }

    async stop(){
        if(this.state in { stopping: 1, standby: 1 })
            return this.state;

        if(this.state == 'starting')
            return await retryShortly(() => this.stop());

        this.state = 'stopping';

        const serverClosePromise = this.#_serverHandles
            ? this.#_serverHandles.close()
            : Promise.resolve();

        // Handle exceptions in user code to maintain proper app state
        try{
            await this._shutdown?.(this);
        }
        catch(err){
            throw err;
        }
        finally{
            await serverClosePromise;
            this.log.info({ type: 'app' }, 'Stopped');
            this.state = 'standby';
            if(typeof this._clearGlobalErrorCatchers == 'function')
                this._clearGlobalErrorCatchers();
        }

        return this.state;
    }

    async trigger(method, path, input = {}){

        if(!(input.body instanceof Readable)){
            const ob = input.body ?? '';
            const oh = input.headers ?? {};
            input = { ...input };

            const type = getContentTypeFromDataType(ob);
            let inputNodeStream;
            if(ob instanceof Buffer)
                inputNodeStream = Readable.from(ob);
            else if(typeof ob == 'object')
                inputNodeStream = Readable.from(Buffer.from(JSON.stringify(ob)));
            else
                inputNodeStream = Readable.from(Buffer.from(String(ob)));

            if(inputNodeStream)
                input.reqStream = Readable.toWeb(inputNodeStream);

            if(!oh['content-type'] && !oh['Content-Type'] && type)
                input.headers = { ...oh, 'content-type': type };
        }
        else
            input.reqStream = Readable.toWeb(input.body);

        const chunks = [];
        input.resStream = new WritableStream({
            write(chunk) {
                console.log('chunk', chunk, typeof chunk);
                chunks.push(chunk);
            }
        });

        const resInfo = await this._api.trigger(method, path, input);
        if(chunks.length > 0){
            const ct = /** @type {string} */ (resInfo.headers?.['content-type'] ?? resInfo.headers?.['Content-Type']);
            const { type, charset } = getDataTypeFromContentType(ct);
            const totalLength = chunks.reduce((total, chunk) => total + chunk.length, 0);
            const merged = new Uint8Array(totalLength);
            let offset = 0;
            for(const chunk of chunks) {
                merged.set(chunk, offset);
                offset += chunk.length;
            }
            resInfo.body = getDataFromBytesAndTypes(merged, type, charset);
        }

        return resInfo;
    }

    async restart(conf){
        await this.stop();
        if(typeof conf == 'object'){
            this.log.debug({ type: 'app' }, 'Reloaded settings');
            this.setup(conf);
        }
        await this.start();
    }

    async run(opts = {}){
        this.setup(...[].concat(opts.conf));

        /* istanbul ignore next */
        const term = () => {
            this.stop();
            setTimeout(() => process.exit(0), 2000);
        }

        /* istanbul ignore next */
        const die = err => {
            this.log.fatal({ err, type: 'crash' });
            process.env.NODE_ENV !== 'production' &&
                console.log(err);
            process.exit(1);
        }

        process.on('SIGINT', term);
        process.on('SIGTERM', term);
        process.on('uncaughtException', die);
        process.on('unhandledRejection', die);

        this._clearGlobalErrorCatchers = function(){
            process.removeListener('uncaughtException', die);
            process.removeListener('unhandledRejection', die);
            process.removeListener('SIGINT', term);
            process.removeListener('SIGTERM', term);
        }

        await this.start();

        return this;
    }

}

const METHODS = [ 'get', 'post', 'delete', 'put', 'patch' ];

METHODS.forEach(m => module.exports[m] = function(path, handler, opts){
    if(typeof handler != 'function')
        throw new TypeError(`'${m.toUpperCase()}' handler must be a function. Found '${typeof handler}'`);
    return { ...opts, method: m, path, handler };
});

// Needed because it's not possible to call a function named 'delete'
module.exports.del = module.exports['delete'];

module.exports.all = handler => {
    if(typeof handler != 'function')
        throw new TypeError(`'ALL' handler must be a function. Found '${typeof handler}'`)
    return { all: true, handler }
};
