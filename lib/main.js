...
module.exports = class Nodecaf {
    ...
    setup(...objectOrPath){
        ...
        if(this.conf.log.only)
            this.log?.warn('Setting `conf.log.only` is deprecated. This option will be dropped on `v0.14.0`. Use external filtering instead.');

        if(this.conf.log.except)
            this.log?.warn('Setting `conf.log.except` is deprecated. This option will be dropped on `v0.14.0`. Use external filtering instead.');
