...
class Nodecaf {
     ...
     * @this {Nodecaf}
     * @param {...Object|string} objectOrPath Configuration object or path
     * @returns {void}
     */
    setup(...objectOrPath){
     ...
     * @param {string} path
     * @param {Object} [input]
     * @param {string|null|boolean|Uint8Array|Object|number} [input.body]
    ...
    async trigger(method, path, input = {}){
        ...
        if(!(input.body instanceof Readable)){
            ...
            if(inputNodeStream)
                reqStream = Readable.toWeb(inputNodeStream);

            if(!oh['content-type'] && !oh['Content-Type'] && type)
                input.headers = { ...oh, 'content-type': type };
        }
        else
            reqStream = Readable.toWeb(input.body);

        const chunks = [];
        const resStream = new WritableStream({
            write(chunk) {
