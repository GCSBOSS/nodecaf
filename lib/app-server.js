...
module.exports = class AppServer {
    ...
    async start(){
        ...
        await this.beforeStart({ conf: this.settings });
        ...
        this.log.info({ conf: this.settings }, 'SERVER START');
    }
    ...
    async stop(){
        ...
        this.log.info('SERVER STOP');

        // Execute after Stop handler.
        await this.afterStop({ conf: this.settings });
    ...
    async restart(){
        await this.stop();
        if(this.settings.debug){
            this.log.info({ conf: this.settings }, 'SERVER RELOAD');
            this.setup({});
        }
        await this.start();
    }
