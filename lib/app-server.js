...
module.exports = class Nodecaf {
    ...
    setup(objectOrPath){
        ...
        this.conf.port = this.conf.port || (this.ssl ? 443 : 80);
    ...
    async start(){
        ...
        await new Promise( done => this.server.listen(this.conf.port, done) );
        this.running = true;

        this.log.info({ type: 'server' },
        	'Started %s (v%s) at port %s', this._name, this._version, this.conf.port);
