/**
 * @module router
 * @description A high-performance trie-based router for managing HTTP routes with static and dynamic path segments.
 * Supports O(1) static route matching and efficient dynamic parameter extraction, including named parameters and wildcards.
 * @example
 ...
 *  handler(ctx, params); // params.id will be '123'
 * }
 * router.add('GET', '/files/...path', (ctx) => { ... });
 * const match = router.match('GET', '/files/images/photo.jpg');
 * if(match) {
 *  const { handler, params } = match;
 * handler(ctx, params); // params.path will be 'images/photo.jpg'
 * }
 */

/**
 * @typedef Trie
 * @property {Object.<string, Trie>} static Static children nodes
 * @property {Trie|null} param Named parameter child node
 * @property {Trie|null} wildcard Wildcard child node
 * @property {Function|null} handler Route handler function
 * @property {string|null} paramName Named parameter or wildcard name
 */

/**
 * Creates a new trie node
 * @returns {Trie} A new trie node
 */
function createNode() {
    return {
...
export class Router {
    ...
    list(){
        return Object.keys(this.#_duplicateCheck).map(r => {
