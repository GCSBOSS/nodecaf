...
function setVaryHeader(res, newValue) {
    ...
    if(!curVal) 
        return res.set('Vary', lowerNew);

    const lowerCur = curVal.toLowerCase();

    // Ensure we don't match partial strings (e.g., matching 'Origin' in 'X-Origin')
    const index = lowerCur.indexOf(lowerNew);
    const isPresent = index !== -1 &&
        (index === 0 || lowerCur[index - 1] === ' ' || lowerCur[index - 1] === ',') &&
        (index + lowerNew.length === lowerCur.length || lowerCur[index + lowerNew.length] === ',');

    if(!isPresent) 
        res.set('Vary', curVal + ', ' + lowerNew);
}
