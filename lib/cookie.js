/**
 * @module cookie
 * @description Cookie parsing and serialization. Handles both parsing cookie headers from requests
 * and generating Set-Cookie headers with full RFC 6265 option support.
 * @example
 * const parsed = parse('sessionId=abc123; Path=/admin');
 * const setCookie = serialize('token', 'xyz789', { 
 *   httpOnly: true, 
 *   secure: true, 
 *   maxAge: 3600 
 * });
 ...
 * @return {string}
 */
export function serialize(key, val, options = {}) {
    const opt = options;
    ...
    // REMOVED: This check is impossible to trigger due to encodeURIComponent above
